  ...
  config.events.forEach(async event => {
    ...
    let promises = event.users.map(async user => {
      ...
      flights.forEach((flight, i) => {
        ...
        txt += `ℹ️ Status: ${flight.status} | 🛄 Baggage claim: ${flight.baggage_claim || "None"} | Gate at ${flight.origin?.code_iata}: ${flight.gate_origin} | Gate at ${flight.destination?.code_iata}: ${flight.gate_destination} \n`
        txt += `🛫 Take off (scheduled): ${flight.scheduled_off ? new Date(flight.scheduled_off).toLocaleString('en-US', { timeZone: event.timezone, timeStyle: "short", dateStyle: "long" }) : "Unknown"} `
        txt += `(Actual: ${flight.actual_off ? new Date(flight.actual_off).toLocaleString('en-US', { timeZone: event.timezone, timeStyle: "short", dateStyle: "long" }) : "Unknown"})\n`
        txt += `🛬 Landing (scheduled): ${flight.scheduled_in ? new Date(flight.scheduled_in).toLocaleString('en-US', { timeZone: event.timezone, timeStyle: "short", dateStyle: "long" }) : "Unknown"} `
        txt += `(Actual: ${flight.actual_in ? new Date(flight.actual_in).toLocaleString('en-US', { timeZone: event.timezone, timeStyle: "short", dateStyle: "long" }) : "Unknown"})\n`
