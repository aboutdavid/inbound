    ...
    config.events.forEach(async event => {
      ...
      let promises = event.users.map(async user => {
        ...
        flights.forEach((flight, i) => {
          ...
          txt += `ℹ️ Status: ${flight.status} | 🛄 Baggage claim: ${flight.baggage_claim || "None"} | Gate at ${flight.origin?.code_iata}: ${flight.gate_origin || "Unknown"} | Gate at ${flight.destination?.code_iata}: ${flight.gate_destination || "Unknown"} \n`
